...
public class SystemBarUtil {
     ...
     * @param context
     * @date 2018/8/12
     *  配合JNavigationbar使用，传入的view 是预留的控件，前提是activity已经是全屏
     */
    public static void setChenJinTitle(View view, Context context) {
        immersiveStatusBar((Activity) context,0);
        int currentapiVersion=android.os.Build.VERSION.SDK_INT;
        if (currentapiVersion >= 19) {
            view.setVisibility(View.VISIBLE);
            int statusBarHeight =getStatusBarHeight(context);//状态栏高度
            if (null != view.getLayoutParams() && view.getLayoutParams() instanceof LinearLayout.LayoutParams) {
                LinearLayout.LayoutParams linearParams = (LinearLayout.LayoutParams) view.getLayoutParams(); // 取控件当前的布局参数
                linearParams.height = statusBarHeight;// 当控件的高强制设成
                view.setLayoutParams(linearParams); // 使设置好的布局参数应用到控件
            } else if (null != view.getLayoutParams() && view.getLayoutParams() instanceof RelativeLayout.LayoutParams) {
                RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) view.getLayoutParams();// 取控件当前的布局参数
                int w = View.MeasureSpec.makeMeasureSpec(0,
                        View.MeasureSpec.UNSPECIFIED);
                int h = View.MeasureSpec.makeMeasureSpec(0,
                        View.MeasureSpec.UNSPECIFIED);
                view.measure(w, h);
                int endHeight = view.getMeasuredHeight() + statusBarHeight;
                layoutParams.height = endHeight;// 当控件的高强制设成
                view.setLayoutParams(layoutParams); // 使设置好的布局参数应用到控件
